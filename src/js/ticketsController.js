...
export default class TicketsController {
  ...
  startElementListeners() {
    this.addTicketButton.addEventListener("click", (e) =>
      this.inputFormShow(e)
    );
  ...
  putData = (data) => {
    if (data.length > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        ticketsArray[i].querySelector(".ticket-name").textContent =
          data[i].name;
        ticketsArray[i].firstElementChild.firstElementChild.style.visibility =
          data[i].status === "true" ? "visible" : "hidden";
        ticketsArray[i].querySelector(".ticket-date").textContent = new Date(
          data[i].created
        )
          .toLocaleString()
          .replace(",", "")
  ...
  changeTicketStatus = (id, status = false) => {
    ...
    formData.append(
      "name",
      document.getElementById(id).firstElementChild.children[2].textContent
    );
  ...
  changeTicket = (id) => {
    ...
    if (this.addTicketFormText.value !== "") {
      ...
      formData.append(
        "status",
        document.getElementById(id).firstElementChild.firstElementChild.style
          .visibility === "visible"
          ? "true"
          : "false"
      );
  ...
  inputFormShow = (e, id = undefined) => {
    ...
    this.addTicketFormCancelButton.addEventListener(
      "click",
      this.inputFormClose
    );
  ...
  deleteDivShow = (ticket) => {
    ...
    if (!this.deleteDiv) {
      ...
      this.deleteDivText.textContent =
      ...
      editImg.onload = () => {
        editImg.addEventListener("click", (e) => {
          ...
          setTimeout(() => {
            ...
            this.addTicketFormText.value =
              editImg.parentElement.children[2].textContent;
